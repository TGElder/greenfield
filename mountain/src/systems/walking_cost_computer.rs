...
use crate::network::walking::{WalkingInNetwork, WalkingNetwork};
...
fn compute_costs(terrain: &Grid<f32>, piste: &Piste, lifts: &HashMap<usize, Lift>) -> PisteCosts {
    ...
    let network = WalkingNetwork {
        terrain,
        reserved: &terrain.map(|_, _| false),
    };
    let piste_positions = piste_positions(piste);
    let network = WalkingInNetwork::for_positions(&network, &piste_positions);
    ...
    for (lift, Lift { from, .. }) in lifts {
        ...
        if grid.in_bounds(from) && grid[from] {
            ...
            let coverage =
                costs.len() as f32 / (piste_positions.len() * DIRECTIONS.len() * 2) as f32;
            println!("INFO: Walking coverage for lift {} = {}", lift, coverage);
...
fn compute_costs_for_position(
    network: &WalkingInNetwork,
    position: &XY<u32>,
) -> HashMap<State, u64> {
    let states_iter = walking_states_for_position(*position);
    network.costs_to_target(&HashSet::from_iter(states_iter))
}

fn walking_states_for_position(position: XY<u32>) -> impl Iterator<Item = State> {
    DIRECTIONS
        .iter()
        .copied()
